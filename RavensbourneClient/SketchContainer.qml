...
Item {
    ...
    Flickable {
        id: sketchScroller
        anchors.fill: parent
        contentWidth: sketchContainer.width
        contentHeight: sketchContainer.height

        onMovementEnded: {
            console.log( 'sketchScroller.contentX : ' + contentX + ' sketchScroller.contentX : ' + contentY );
        }

        PinchArea {
            id: sketchZoom
            width: Math.max(sketchScroller.contentWidth, sketchScroller.width)
            height: Math.max(sketchScroller.contentHeight, sketchScroller.height)
            ...
            onPinchStarted: {
                initialWidth = sketchContainer.width
                initialHeight = sketchContainer.height
                initialScale = sketchContainer.scale
            }

            onPinchUpdated: {
                sketchContainer.scale = initialScale * pinch.scale;
            }

            onPinchFinished: {
                // Move its content within bounds.
                sketchScroller.returnToBounds()
            }
            Item {
                id: sketchContainer
                width: 1920
                height: 1080
                scale: Math.min( 1.0, Screen.width / 1920. )
                //
                //
                //
                MouseArea {
                    ...
                    onClicked: {
                        ...
                        if ( tool === "text" || tool === "image" ) {
                            if ( activeEditor ) {
                                ...
                                console.log( 'createSketckItem '  + tool + ' at [' + (mouse.x*sketchContainer.scale) + ',' + (mouse.y*sketchContainer.scale) + ']' )
                ...
                Item {
                    ...
                    onWidthChanged: {
                        console.log( 'sketch.onWidthChange : ' + sketch.width + ',' + sketch.height );
                    }
                }
                Drawing {
                    id: drawing
    ...
    */
    ...
    FontChooser {
        ...
        onFontChanged: {
            if ( activeEditor.type === "text" ) {
                activeEditor.setFontStyle( font, colour );
            }
        }
    }
    //
    ...
    Component.onCompleted: {
        /*
        var initialScale = Math.min( 1.0, Screen.width / 1920. );
        sketchScroller.contentX = ( Screen.width - (initialScale * 1920.) ) / 2.;
        sketchScroller.contentY = ( Screen.height - (initialScale * 1080.) ) / 2.;
        console.log( 'sketchScroller.contentX : ' + sketchScroller.contentX + ' sketchScroller.contentY : ' + sketchScroller.contentY )
        */
        if ( Screen.width < sketchScroller.contentWidth ) {
            sketchScroller.contentX = ( sketchScroller.contentWidth - Screen.width ) / 2.;
        }
        if ( Screen.height < sketchScroller.contentHeight ) {
            sketchScroller.contentY = ( sketchScroller.contentHeight - Screen.height ) / 2.;
        }
        console.log( 'sketchScroller.contentX : ' + sketchScroller.contentX + ' sketchScroller.contentY : ' + sketchScroller.contentY );
    }
    ...
    //
    //
    property var drawColour: lineStyleChooser.getColour()
    property var drawLineWidth: lineStyleChooser.getWidth()
    ...
    function setTool( newTool, param ) {
        ...
        */
        lineStyleChooser.visible = false;
        fontStyleChooser.visible = false;
        enableSketchItems();
        drawingLine = false;
