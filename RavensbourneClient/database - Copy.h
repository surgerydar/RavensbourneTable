...
private:
    ...
    void del( const QString& command, const QStringList parameters );
    QString formatParameters( const QStringList parameters );
    QByteArray formatPayload( const QVariantMap data );
    void send( const QString& method, const QString& command, const QStringList parameters, const QByteArray& payload = QByteArray() );
    //
    //
    //
