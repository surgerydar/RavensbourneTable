...
void FingerprintScanner::deleteEntry(const QString& name) {
    std::vector< FeatureTemplate >::iterator it = m_templates.begin();
...
void FingerprintScanner::enroll(QString device) {
    ...
    //
    //
    int status = -1;//dpfj_start_enrollment(DPFJ_FMD_DP_REG_FEATURES);
    if ( true ) {//!InterpretDPFJStatus(status) ) {
        m_mode = VALIDATION;
        m_enrollment_stage = 0;
        emit enrollmentFailed(device, status);
    } else {
