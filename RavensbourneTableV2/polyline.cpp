...
void PolyLine::rotate(const QVector2D& cp, const float angle ) {
    float s = sin(angle);
    float c = cos(angle);
    for( auto& point : m_points ) {
        QVector2D temp = point - cp;
        point.setX( ( temp.x() * c - temp.y() * s ) + cp.x() );
        point.setY( ( temp.x() * s + temp.y() * c ) + cp.y() );
    }
    m_hasChanged = true;
}

void PolyLine::scale(const QVector2D& cp, const float scale ) {
    for( auto& point : m_points ) {
        point -= cp;
        point *= scale;
        point += cp;
...
QRectF PolyLine::getBounds() {
    ...
    return QRectF( min_x - m_lineWidth/2., min_y - m_lineWidth/2., (max_x - min_x ) + m_lineWidth, ( max_y - min_y ) + m_lineWidth );
}
//
//
//
